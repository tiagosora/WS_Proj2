...
def generate_wikidata_queries(resources):
    ...
    for (source, resource_name) in resources:
        query = f"""
        SELECT ?school ?p ?pLabel ?o ?oLabel
        WHERE {{
        ?school wdt:P31 wd:Q2043034 .
...
def save_graph(graph):
    graph.serialize(destination="./output_schools.owl", format='xml')

# Load existing ontology
print("Loading ontology...")
ontology_graph = load_ontology(ONTOLOGY_FILE)

# Get existing resources and their names for the class "School"
print("Retrieving existing resources...")
resources = get_existing_resources_and_names(ontology_graph, "School")
