...
def generate_wikidata_queries(resources):
    ...
    for (source, resource_name) in resources:
        query = f"""
        SELECT ?house ?p ?pLabel ?o ?oLabel
        WHERE {{
        ?house wdt:P31 wd:Q933971 .
...
def save_graph(graph):
    graph.serialize(destination="./output_houses.owl", format='xml')

# Load existing ontology
print("Loading ontology...")
ontology_graph = load_ontology(ONTOLOGY_FILE)

# Get existing resources and their names for the class "House"
print("Retrieving existing resources...")
resources = get_existing_resources_and_names(ontology_graph, "House")
