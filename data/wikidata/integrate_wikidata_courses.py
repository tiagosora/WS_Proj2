...
def generate_wikidata_queries(resources):
    ...
    for (source, resource_name) in resources:
        query = f"""
        SELECT ?course ?p ?pLabel ?o ?oLabel
        WHERE {{
        ?course wdt:P31 wd:Q1647221 .
...
def save_graph(graph):
    graph.serialize(destination="./output_courses.owl", format='xml')

# Load existing ontology
print("Loading ontology...")
ontology_graph = load_ontology(ONTOLOGY_FILE)

# Get existing resources and their names for the class "Course"
print("Retrieving existing resources...")
resources = get_existing_resources_and_names(ontology_graph, "Course")
